...
BASE_OUTPUT_DIR="./outputs_lokr_alpha_search"
...
# Default LoKR target modules (Placeholder - Adjust if needed for LoKR)
LOKR_TARGET_MODULES='["to_q", "to_k", "to_v", "to_out.0"]'
...
# Common evaluation parameters (copied from LoHa script)
...
for ALPHA in "${ALPHAS[@]}"; do
    ...
    CONFIG_NAME="${ADAPTER_TYPE}_rank_${RANK}_alpha_${ALPHA}" # Assuming 'rank'
    TEMP_ADAPTER_CONFIG_PATH="./temp_adapter_config_${CONFIG_NAME}.yaml" # Relative to PROJECT_DIR

    echo ""
    echo "***************************************"
    echo "Processing Rank: ${RANK}, Alpha: ${ALPHA}" # Assuming 'rank'
    ...
    # IMPORTANT: Adjust fields (rank vs dim) if LoKR config differs from LoHa
    cat << EOF > "$TEMP_ADAPTER_CONFIG_PATH"
adapter_type: ${ADAPTER_TYPE}
rank: ${RANK} # Or maybe 'dim: ${RANK}'
alpha: ${ALPHA}
target_modules: ${LOKR_TARGET_MODULES}
# Add other LoKR specific parameters if needed (e.g., dropout)
EOF
    ...
    echo "======================================="
    echo "Configuration for Rank ${RANK}, Alpha ${ALPHA}" # Assuming 'rank'
    ...
    echo "======================================="
    echo "Processing Object Instances (Rank ${RANK}, Alpha ${ALPHA})" # Assuming 'rank'
    echo "======================================="

    for INSTANCE in "${OBJECT_INSTANCES[@]}"; do
        echo ""
        echo "======================================="
        echo "Processing Instance: ${INSTANCE} (Rank ${RANK}, Alpha ${ALPHA})" # Assuming 'rank'
        echo "======================================="
        ...
        if [ -d "${INSTANCE_MODEL_DIR}" ]; then
            ...
            echo "Skipping training for ${INSTANCE} (Rank ${RANK}, Alpha ${ALPHA})." # Assuming 'rank'
        else
            echo "Starting LoKR Training Run for ${INSTANCE} (Rank ${RANK}, Alpha ${ALPHA})..." # Updated text
            ...
            echo "Training for ${INSTANCE} (Rank ${RANK}, Alpha ${ALPHA}) finished successfully." # Assuming 'rank'
        fi
        ...
        if [ -n "$EVAL_DIRS" ] && [ -n "$(find $EVAL_DIRS -type f -name "*.png" -o -name "*.jpg" | head -n 1)" ]; then
            ...
            echo "======================================="
            echo "Skipping Evaluation for ${INSTANCE} (Rank ${RANK}, Alpha ${ALPHA})" # Assuming 'rank'
            echo "Found existing evaluation results in: ${INSTANCE_OUTPUT_DIR}"
            echo "======================================="
        else
            # If no results found, proceed with evaluation
            echo "======================================="
            echo "Starting Evaluation for ${INSTANCE} (Rank ${RANK}, Alpha ${ALPHA})" # Assuming 'rank'
            echo "======================================="
            ...
            if [ ! -d "$ADAPTER_DIR" ]; then
                echo "Error: Adapter directory ${ADAPTER_DIR} not found. Cannot run evaluation for ${INSTANCE} (Rank ${RANK}, Alpha ${ALPHA})." >&2 # Assuming 'rank'
                # Continue to next instance instead of exiting
                continue
            fi

            # Run evaluation using the trained adapter
            # Pass the experiment config, specific adapter dir, instance info, and common eval params
            ...
            if ! find "$EVALUATION_OUTPUT_DIR" -maxdepth 1 -type f \( -name '*.png' -o -name '*.jpg' \) -print -quit | grep -q .; then
                ...
                echo "Evaluation complete for ${INSTANCE} (Rank ${RANK}, Alpha ${ALPHA})! Results saved to ${EVALUATION_OUTPUT_DIR}" # Assuming 'rank'
            fi
        fi
    done
    ...
    echo ""
    echo "======================================="
    echo "Processing Live Instances (Rank ${RANK}, Alpha ${ALPHA})" # Assuming 'rank'
    echo "======================================="
...
echo ""
echo "======================================="
echo "All LoKR alpha values processed successfully (Rank ${RANK})" # Updated text, assuming 'rank'
