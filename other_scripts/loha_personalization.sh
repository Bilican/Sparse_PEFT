# --- Configuration --- #
# Define the LoHa alpha values to test
#ALPHAS=(2 4 8 16 32 64 128 256 512)
ALPHAS=(64 128)
# Define the fixed LoHa rank for this search
RANK=1
# Base directory for all outputs (Updated for LoHa alpha search)
BASE_OUTPUT_DIR="./outputs_loha_alpha_search"
...
# Default LoHa target modules (from loha_config.yaml)
LOHA_TARGET_MODULES='["to_q", "to_k", "to_v", "to_out.0"]'
...
echo "======================================="
echo "Starting LoHa Alpha Hyperparameter Search"
echo "Fixed Rank: ${RANK}"
...
# Assuming default dropout/bias if needed, add them here if not.
# module_dropout: 0.0
# bias: "none"
EOF
    ...
    for INSTANCE in "${OBJECT_INSTANCES[@]}"; do
        ...
        if [ -d "${INSTANCE_MODEL_DIR}" ]; then
            ...
            echo "Starting LoHa Training Run for ${INSTANCE} (Rank ${RANK}, Alpha ${ALPHA})..."
...
echo ""
echo "======================================="
echo "All LoHa alpha values processed successfully (Rank ${RANK})" # Updated text
